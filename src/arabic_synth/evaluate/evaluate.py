...
def _eval_exams(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def _eval_sentiment(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def _eval_grammar(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def run_evaluation(task: str, in_path: Path) -> Dict[str, Any]:
    ...
    if task == "exams":
        return _eval_exams(rows)
    if task == "sentiment":
        return _eval_sentiment(rows)
    if task == "grammar":
        return _eval_grammar(rows)
