# -*- coding: utf-8 -*-
"""
MadinahQA data processing module that handles:
1. CSV parsing and sampling (uniform/stratified)
2. Format conversion (CSV to JSONL)
3. Unified interface for MadinahQA data operations
"""
...
class MadinahQAProcessor:
    """
    Unified MadinahQA data processor that handles CSV parsing, sampling, and format conversion.
    """
    
    def __init__(self, madinah_file: str):
        if not madinah_file.endswith(".csv"):
            raise ValueError("This version only supports CSV format for MadinahQA dataset")
        self.file = madinah_file
        self.data = None
        self._load_data()
    ...
    def save_csv(self, df: pd.DataFrame, out_file: str):
        ...
        # Save with headers, preserving the original format
    ...
    def convert_to_jsonl(self, input_csv: Optional[str] = None, output_jsonl: Optional[str] = None) -> Dict[str, int]:
        ...
        out_jsonl = output_jsonl or "data/madinahqa.jsonl"
    ...
    def sample_and_convert(self, n: int, mode: str = "uniform", stratify_col: str = "Subject", 
        ...
        temp_csv = "temp_madinahqa_sampled.csv"
        self.save_csv(sampled_df, temp_csv)
        
        # Convert to JSONL
        out_jsonl = output_jsonl or f"outputs/madinahqa_{args.mode}_{args.n}.jsonl"
...
def sample_uniform_main(input_file=None, output_file=None, n=10, seed=None):
    ...
    input_file = input_file or "data/MadinahQA/merged_madinah_qa.csv"
    output_file = output_file or "outputs/madinahqa_uniform.csv"
    
    processor = MadinahQAProcessor(input_file)
...
def sample_stratified_main(input_file=None, output_file=None, n=20, stratify_col="Subject", seed=None):
    ...
    input_file = input_file or "data/MadinahQA/merged_madinah_qa.csv"
    output_file = output_file or "outputs/madinahqa_stratified.csv"
...
def convert_csv_main(input_csv=None, output_jsonl=None):
    ...
    in_csv = input_csv or "data/MadinahQA/merged_madinah_qa.csv"
    out_jsonl = output_jsonl or "data/madinahqa.jsonl"
    
    processor = MadinahQAProcessor(in_csv)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process MadinahQA datasets: sample and convert formats")
    ...
    parser.add_argument("--input-file", default="data/MadinahQA/merged_madinah_qa.csv",
                       help="Input CSV file")
    ...
    processor = MadinahQAProcessor(args.input_file)
    
    if args.operation == "sample":
        # Sample only, save to CSV
        if args.mode == "uniform":
            output_file = args.output_csv or "outputs/madinahqa_uniform.csv"
            sample_uniform_main(
            ...
            output_file = args.output_csv or "outputs/madinahqa_stratified.csv"
        ...
        # Convert only
        output_jsonl = args.output_jsonl or "data/madinahqa.jsonl"
        ...
        output_jsonl = args.output_jsonl or f"outputs/madinahqa_{args.mode}_{args.n}.jsonl"
